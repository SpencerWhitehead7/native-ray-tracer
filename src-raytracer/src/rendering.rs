...
fn get_color(scene: &Scene, ray: &Ray, intersection: &Intersection, depth: u32) -> Color {
  ...
  match material.surface {
    ...
    SurfaceType::Reflective { reflectivity } => {
      ...
      let reflection_ray = Ray::create_reflection(normal, ray.direction, hit, scene.shadow_bias);
    ...
    SurfaceType::Refractive {
      ...
      if kr < 1.0 {
        let transmission_ray =
          Ray::create_transmission(normal, ray.direction, hit, scene.shadow_bias, index).unwrap();
